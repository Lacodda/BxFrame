   ...
   :  "-.  "-:  ;          :/."      .\'  :
     ...
     ';
        ...
        protected function createEnvironmentsDir (InputInterface $input, OutputInterface $output)
        {
            ...
            if (file_exists ($targetDir))
            {
                $question = new ConfirmationQuestion(
                    '    <error>Directory ' . $targetDir . ' already exists</error>' . PHP_EOL . '    <info>Overwrite? [Y/n]</info> ',
                    true,
                    '/^(y|j)/i'
                );
        ...
        protected function createConfiguration (InputInterface $input, OutputInterface $output)
        {
            ...
            {
                $question = new ConfirmationQuestion(
                    '    <error>Configuration file ' . $path . ' already exists</error>' . PHP_EOL . '    <info>Overwrite? [Y/n]</info> ',
                    true,
                    '/^(y|j)/i'
                );
            ...
            $question = new Question(
                '    <info>Enter path to web directory relative to ' .
                $this->getApplication ()
                     ->getRoot () .
                ':</info> ' .
                PHP_EOL .
                '    (or do not specify if you are already in the web directory)' .
                PHP_EOL
            );

            $question->setValidator (
                function ($answer) use ($fs)
                {
                    $path = $answer;
