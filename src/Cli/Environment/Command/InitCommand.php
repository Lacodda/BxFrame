<?php
    ...
    namespace Lacodda\BxFrame\Cli\Environment\Command;
    ...
    use Lacodda\BxFrame\Cli\Application\Command\Command;
    use Lacodda\BxFrame\Cli\Application\Exception\BitrixException;
    use Lacodda\BxFrame\Cli\Module\Command\ModuleCommand;
    use Lacodda\BxFrame\Cli\Module\Exception\ModuleException;
    use Lacodda\BxFrame\Cli\Module\Exception\ModuleInstallException;
    use Lacodda\BxFrame\Cli\Module\Module;
        ...
        extends Command
    {
        ...
        protected function initialize (InputInterface $input, OutputInterface $output)
        {
            ...
            if (!$this->getApplication ()
                      ->getConfiguration ()['env-dir']
            )
            {
                throw new \Exception('Config env-dir is missing');
            }

            $dir = $this->getApplication ()
                        ->getRoot () .
                '/' .
                $this->getApplication ()
                     ->getConfiguration ()['env-dir'];
        ...
        protected function copyFiles (InputInterface $input, OutputInterface $output)
        {
            ...
            foreach ($iterator as $item)
            {
                ...
                $itemPath = $this->getApplication ()
                                 ->getRoot () . '/' . $iterator->getSubPathName ();
        ...
        protected function initializeBitrix ()
        {
            ...
            foreach ($connections as $name => $parameters)
            {
                Application::getInstance ()
                           ->getConnectionPool ()
                           ->cloneConnection ($name, $name, $parameters);
            }
        }
        ...
        protected function configureModules (InputInterface $input, OutputInterface $output, array $modules)
        {
            ...
            if ($app->getConfiguration ())
            {
                ...
                if ($app->getBitrixStatus () != \Lacodda\BxFrame\Cli\Application\Application::BITRIX_STATUS_COMPLETE)
                {
