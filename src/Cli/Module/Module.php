<?php
    ...
    namespace Lacodda\BxFrame\Cli\Module;

    use Bitrix\Main\Config\Option;
    use Bitrix\Main\ModuleManager;
    use Lacodda\BxFrame\Cli\Application\Exception\BitrixException;
    ...
    class Module
    {
            ...
            if (!$this->isRegistered ())
            {
                ...
                if ((new \ReflectionClass($moduleObject))->getMethod ('InstallDB')->class !== get_class ($moduleObject))
                {
                    throw new Exception\ModuleInstallException(
                        'Missing InstallDB method. This module does not support automatic installation', $this->name
                    );
                }
                ...
                {
                    throw new Exception\ModuleInstallException(
                        get_class ($moduleObject) . '::InstallDB() returned false', $this->name
                    );
                }
                ...
                {
                    throw new Exception\ModuleInstallException(
                        get_class ($moduleObject) . '::InstallFiles() returned false', $this->name
                    );
                }
                ...
                {
                    throw new Exception\ModuleInstallException(
                        'Module was not registered. Probably it does not support automatic installation.', $this->name
                    );
                }
            }
            ...
            if ($this->isRegistered ())
            {
                ...
                if ((new \ReflectionClass($moduleObject))->getMethod ('UnInstallDB')->class !== get_class ($moduleObject))
                {
                    throw new Exception\ModuleUninstallException(
                        'Missing UnInstallDB method. This module does not support automatic uninstallation', $this->name
                    );
                }
                ...
                {
                    throw new Exception\ModuleUninstallException(
                        get_class ($moduleObject) . '::UnInstallFiles() returned false', $this->name
                    );
                }
        ...
        public function update (&$modulesUpdated = null)
        {
            ...
            if (!$this->isThirdParty ())
            {
                throw new Exception\ModuleUpdateException(
                    'Kernel module updates are currently not supported.', $this->getName ()
                );
            }
            ...
            {
                throw new Exception\ModuleUpdateException(
                    '[CL02] UnGzipArchive failed. ' . $errorMessage, $this->getName ()
                );
            }
            ...
            {
                throw new Exception\ModuleUpdateException(
                    '[CL04] UpdateStepModules failed. ' . $errorMessage, $this->getName ()
                );
            }
        ...
        protected function validateUpdate ($updateDir)
        {
            ...
            {
                throw new Exception\ModuleUpdateException(
                    '[CL03] CheckUpdatability failed. ' . $errorMessage, $this->getName ()
                );
            }
