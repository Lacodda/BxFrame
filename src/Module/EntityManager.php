<?php
    ...
    namespace Lacodda\BxFrame\Module;
    ...
    class EntityManager
    {
        ...
        public function delete ()
        {
            ...
            if (!$result->isSuccess ())
            { // если хотя бы одна из них не удалилась
        ...
        protected function collectReferencesData ()
        {
            ...
            foreach ($references as $fieldName => $reference)
            {
                if (array_key_exists ($fieldName, $this->data))
                {
                    ...
                    {
                        $result->addError (new Entity\EntityError($this->lang->getMessage ('RELATION_SHOULD_BE_MULTIPLE_FIELD')));
            ...
            foreach ($this->referencesData as $fieldName => $referenceDataSet)
            {
                ...
                { // Удаление записей, которые не были созданы или обновлены
                    foreach ($referenceStaleDataSet as $referenceData)
                    {
                        ...
                        {
                            $result = $this->deleteReferenceData (
                                $reference,
        ...
        protected function createReferenceData (Entity\ReferenceField $reference, array $referenceData)
        {
            ...
            if (!empty($referenceData[$fieldWidget->getMultipleField ('ID')]))
            {
                throw new ArgumentException($this->lang->getMessage ('ARGUMENT_CANT_CONTAIN_ID', array ('%A%' => 'referenceData')), 'referenceData');
            }

            $refClass = $reference->getRefEntity ()
                                  ->getDataClass ();
            ...
            {
                $this->addNote (
                    $this->lang->getMessage (
                        'RELATION_SAVE_ERROR',
                        array ('#FIELD#' => $fieldParams['TITLE'])
                    ),
         ...
         * @throws ArgumentException
         */
        protected function updateReferenceData (Entity\ReferenceField $reference,
                                                array $referenceData,
                                                array $referenceStaleDataSet)
        {
            ...
            if (empty($referenceData[$fieldWidget->getMultipleField ('ID')]))
            {
                throw new ArgumentException(
                    $this->lang->getMessage ('ARGUMENT_SHOULD_CONTAIN_ID', array ('%A%' => 'referenceData')), 'referenceData'
                );
            }

            // Сравнение старых данных и новых, обновляется только при различиях
            if ($this->isDifferentData (
                $referenceStaleDataSet[$referenceData[$fieldWidget->getMultipleField ('ID')]],
                $referenceData
            )
            )
            {
