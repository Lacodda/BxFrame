<?php
     ...
     */
    abstract class AdminEditHelper
        extends AdminBaseHelper
    {
        ...
        public function __construct (array $fields, array $tabs = array ())
        {
            ...
            if (empty($this->tabs))
            {
                $this->tabs = array (
                    array (
                        'DIV'     => 'DEFAULT_TAB',
                        'TAB'     => $this->lang->getMessage ('DEFAULT_TAB'),
                        'ICON'    => 'main_user_edit',
                        'TITLE'   => $this->lang->getMessage ('DEFAULT_TAB'),
                        'VISIBLE' => true,
                    ),
                );
            } else
            ...
            } else
            {
                ...
                {
                    if (isset($helperFields[$field]['VIRTUAL']) AND
                        $helperFields[$field]['VIRTUAL'] == true AND
                        (!isset($helperFields[$field]['FORCE_SELECT']) OR $helperFields[$field]['FORCE_SELECT'] = false)
                    )
                    {
                        unset($select[$key]);
                    }
        ...
        protected function getMenu ($showDeleteButton = true)
        {
            ...
            if ($showDeleteButton && isset($this->data[$this->pk ()]) && $this->hasDeleteRights ())
            {
                $arSubMenu[] = $this->getButton (
                    'DELETE_ELEMENT',
                    array (
                        'ONCLICK' => "if(confirm('" . $this->lang->getMessage ('EDIT_DELETE_CONFIRM') . "')) location.href='" . static::getUrl (
                                array_merge (
                                    $this->additionalUrlParams,
                                    array (
                                        'ID'            => $this->data[$this->pk ()],
        ...
        protected function editAction ()
        {
            ...
            {
                $this->addErrors ($this->lang->getMessage ('EDIT_WRITE_FORBIDDEN'));
        ...
        protected function deleteElement ($id)
        {
            ...
            {
                $this->addErrors ($this->lang->getMessage ('EDIT_DELETE_FORBIDDEN'));
        ...
        protected function setElementTitle ()
        {
            if (!empty($this->data))
            {
                $title = $this->lang->getMessage ('EDIT_TITLE', array ('#ID#' => $this->data[$this->pk ()]));
            } else
            {
                $title = $this->lang->getMessage ('NEW_ELEMENT');
            }
