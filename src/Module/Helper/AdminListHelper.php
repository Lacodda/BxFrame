<?php
        ...
        extends AdminBaseHelper
    {
         ...
         */
        static protected $tablePrefix = "lacodda_bxframe_";
        ...
        protected function prepareAdminVariables ()
        {
            ...
            foreach ($this->fields as $code => $settings)
            {
                ...
                if (($sectionField != $code && $widget->getSettings ('FILTER') !== false) &&
                ...
                )
                {
                    ...
                    if (isset($_REQUEST[$filterVarName]) AND !isset($_REQUEST['del_filter']) AND $_REQUEST['del_filter'] != 'Y')
                    {
                ...
                if (!isset($settings['HEADER']) OR $settings['HEADER'] != false)
                {
                    ...
                    if (!$mergedColumn)
                    {
                        $this->arHeader[] = array (
                            "id"                     => $code,
                            "content"                => $widget->getSettings ('LIST_TITLE') ? $widget->getSettings ('LIST_TITLE')
                                : $widget->getSettings ('TITLE'),
                            "sort"                   => $code,
                            "default"                => true,
        ...
        protected function checkFilter ($arFilter)
        {
            ...
            foreach ($this->filterTypes as $code => $type)
            {
                ...
                {
                    $filterValidationErrors = array_merge (
        ...
        protected function getGroupActions ()
        {
            ...
            if (!$this->isPopup ())
            {
                ...
                {
                    $result = array ('delete' => $this->lang->getMessage ("LIST_DELETE"));
                }
            }
        ...
        protected function groupActions ($IDs, $action)
        {
            ...
            if ($action == 'delete')
            {
                ...
                {
                    $this->addErrors ($this->lang->getMessage ('LIST_DELETE_FORBIDDEN'));
                }
            }
        ...
        protected function editAction ($id, $fields)
        {
            ...
            if (strpos ($id, 's') === 0)
            { // для раделов другой класс модели
            ...
            {
                $this->list->AddGroupError ($this->lang->getMessage ("MAIN_ADMIN_SAVE_ERROR"), $id);
            ...
            {
                $fieldList = implode ("\n", $errors);
                $this->list->AddGroupError ($this->lang->getMessage ("MAIN_ADMIN_SAVE_ERROR") . " " . $fieldList, $id);
            }
            ...
            if (!empty($errors))
            {
                ...
                {
                    /** @var \Lacodda\BxFrame\Module\Widget\HelperWidget $widget */
        ...
        public function buildList ($sort)
        {
            ...
            if ($sectionEditHelper) // Вывод разделов и элементов в одном списке
            {
                ...
                $this->list->NavText ($res->GetNavPrint ($this->lang->getMessage ("PAGES")));
        ...
        protected function customNavStart (&$res)
        {
            ...
            $res->NavPageNomer =
                ($res->PAGEN < 1 || $res->PAGEN > $res->NavPageCount ? (\CPageOption::GetOptionString ("main", "nav_page_in_session", "Y") != "Y" ||
                $_SESSION[$res->SESS_PAGEN] < 1 ||
                $_SESSION[$res->SESS_PAGEN] > $res->NavPageCount ? 1 : $_SESSION[$res->SESS_PAGEN]) : $res->PAGEN);
        }
        ...
        protected function getRowActions ($data, $section = false)
        {
            ...
            if ($this->isPopup ())
            {
                ...
                $actions['select'] = array (
                    'ICON'    => 'select',
                    'DEFAULT' => true,
                    'TEXT'    => $this->lang->getMessage ('LIST_SELECT'),
            ...
            } else
            {
                ...
                if ($this->hasWriteRights ())
                {
                    ...
                    $actions['edit'] = array (
                        'ICON'    => 'edit',
                        'DEFAULT' => true,
                        'TEXT'    => $this->lang->getMessage ('LIST_EDIT'),
                        'ACTION'  => $this->list->ActionRedirect (
                            $section ? $sectionHelperClass::getUrl ($query) : $editHelperClass::getUrl ($query)
                        ),
                    );
                }
                if ($this->hasDeleteRights ())
                {
                    $actions['delete'] = array (
                        'ICON'   => 'delete',
                        'TEXT'   => $this->lang->getMessage ("LIST_DELETE"),
                        'ACTION' => "if(confirm('" . $this->lang->getMessage ('LIST_DELETE_CONFIRM') . "')) " . $this->list->ActionDoGroup (
                                $data[$this->pk ()],
        ...
        protected function getFooter ($res)
        {
            return array (
                $this->getButton (
                    ...
                    array ("value" => $res->SelectedRowsCount ())
                ),
                $this->getButton (
        ...
        public function show ()
        {
            ...
            {
                $this->addErrors ($this->lang->getMessage ('ACCESS_FORBIDDEN'));
                $this->showMessages ();
        ...
        {
            if (!isset($_SESSION['LAST_GET_QUERY'][get_called_class ()]) OR !isset($_REQUEST['restore_query']) OR $_REQUEST['restore_query'] != 'Y')
            {
                return;
            }
