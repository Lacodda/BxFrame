<?php
     ...
     * класса \Lacodda\BxFrame\Module\EntityManager.
     *
     * @see    EntityManager
    ...
    abstract class HelperWidget
    {
        ...
        protected function getMultipleEditHtml ()
        {
            ...
            return $this->lang->getMessage ('MULTI_NOT_SUPPORT');
        }
         ...
         *
         * @see $this->lang->getMessage()
         */
        protected function addError ($message, $replace = array ())
        {
            $this->validationErrors[$this->getCode ()] = $this->lang->getMessage (
                $message,
        ...
        {
            if (is_a ($this->helper, 'Lacodda\BxFrame\Module\Helper\AdminListHelper'))
            {
                return self::LIST_HELPER;
            } else
            {
                if (is_a ($this->helper, 'Lacodda\BxFrame\Module\Helper\AdminEditHelper'))
                {
            ...
            <script>
                ...
                MultipleWidgetHelper.prototype={
                    ...
                    _generateFieldContent:  function(fieldTemplate, data){
                        return '<div class="field-container" style="margin-bottom: 5px;">'+this._generateFieldTemplate(
                                fieldTemplate, data
                            )+this._getDeleteButton()+'</div>';
                    },
                    /**
                    ...
                    _trackEvents:           function(){
                        var context=this;
                        // Добавление поля
                        this.$container.find('.add-field-button').on(
                            'click', function(){
                                context.addField();
                            }
                        );
                        // Удаление поля
                        this.$container.on(
                            'click', '.delete-field-button', function(){
