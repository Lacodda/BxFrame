<?php
     ...
     * наследником `\Lacodda\BxFrame\Module\Helper\AdminBaseHelper`.
        ...
        extends NumberWidget
    {
        ...
        public function loadSettings ($code = null)
        {
            ...
            if (!is_subclass_of ($this->getSettings ('HELPER'), '\Lacodda\BxFrame\Module\Helper\AdminBaseHelper'))
            {
                throw new ArgumentTypeException('HELPER', '\Lacodda\BxFrame\Module\Helper\AdminBaseHelper');
            }
        ...
        protected function getEditHtmlSelect ()
        {
            ...
            if (!empty($entityData))
            {
                ...
                $elementName = $entityData[$this->getSettings ('TITLE_FIELD_NAME')] ? $entityData[$this->getSettings ('TITLE_FIELD_NAME')]
                    : $this->lang->getMessage ('IBLOCK_ELEMENT_NOT_FOUND');
            } else
            {
                $elementId = '';
            }
            ...
            '"
                     type="text">' .
            '<input type="button"
                    value="..." onClick="jsUtils.OpenWindow(\'' .
            $popupUrl .
            '\', ' .
            $windowWidth .
            ', ' .
        ...
        public function genEditHtmlInputs ()
        {
            ...
            {
                $return = $this->lang->getMessage ('ORM_MISSING_ELEMENTS');
            }
        ...
        public function getMultipleEditHtml ()
        {
            ...
            <script>
                var multiple=new MultipleWidgetHelper(
                    '#<?= $uniqueId ?>-field-container', '<input name="<?=$key?>[{{field_id}}][VALUE]"'+'id="<?=$name?>[{{field_id}}]"'+'value="{{value}}"'+'size="<?=$inputSize?>"'+'type="text">'+'<input type="button"'+'value="..."'+'onClick="jsUtils.OpenWindow(\'<?=$popupUrl?>\', <?=$windowWidth?>, <?=$windowHeight?>);">'+'&nbsp;<span id="sp_<?=md5 (
                        $name
                    )?>_{{field_id}}" >{{element_title}}</span>'
                );
                <?
                ...
                $elementName = $referenceData[$this->getSettings ('TITLE_FIELD_NAME')] ? $referenceData[$this->getSettings ('TITLE_FIELD_NAME')]
                    : $this->lang->getMessage ('IBLOCK_ELEMENT_NOT_FOUND');
        ...
        public function getValueReadonly ()
        {
            ...
            if (!empty($entityData))
            {
                $entityName = $entityData[$this->getSettings ('TITLE_FIELD_NAME')] ? $entityData[$this->getSettings ('TITLE_FIELD_NAME')]
                    : $this->lang->getMessage ('IBLOCK_ELEMENT_NOT_FOUND');
        ...
        {
            /**
             * @var \Lacodda\BxFrame\Module\Helper\AdminBaseHelper $linkedHelper
             */
            $linkedHelper = $this->getSettings ('HELPER');
