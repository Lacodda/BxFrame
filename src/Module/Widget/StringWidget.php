<?php
    ...
    use Lacodda\BxFrame\Module\Helper\AdminSectionListHelper;
        ...
        protected function getEditHtml ()
        {
            ...
            if ($this->getSettings ('TRANSLIT'))
            {
                ...
                $link = '<image id="' .
                    $nameId .
                    '" title="' .
                    $this->lang->getMessage ("IBSEC_E_LINK_TIP") .
                    '" class="linked" src="/bitrix/themes/.default/icons/iblock/link.gif" onclick="' .
                    $linkedFunctionName .
                    '()" />';
            }
        ...
        protected function getMultipleEditHtml ()
        {
            ...
            <script>
                var multiple=new MultipleWidgetHelper(
                    '#<?= $uniqueId ?>-field-container', '{{field_original_id}}<input type="text" name="<?= $this->getCode (
                    )?>[{{field_id}}][<?=$this->getMultipleField ('VALUE')?>]" style="<?=$style?>" size="<?=$size?>" value="{{value}}">'
                );
                <?
                if ($rsEntityData)
                {
                ...
                multiple.addField(
                    {
                        ...
                        field_original_id: '<input type="hidden" name="<?= $this->getCode ()?>[{{field_id}}][<?= $this->getMultipleField (
                            'ID'
